...
public class Cabip001Steps extends BatchCucumberTester {
	...
	public void accountInEBEDueToBeBilled() throws Throwable {
		Integer test_account = (Integer) (databaseTester.getConnection()
				.createQueryTable("extn_bill_extract", "select account_no from extn_bill_extract")
				.getValue(0, "account_no"));
		this.accountNo = test_account.intValue();

		// Store the prev bill ref no currently in CMF
		ITable cmfBeforeTest = databaseTester.getConnection().createQueryTable("cmf",
				"select prev_bill_refno, prev_bill_ref_resets from cmf where account_no = " + this.accountNo);
		...
		Reporter.addStepLog("Latest invoice number before executing CABIP001: " + this.prevBillRefNo.toString() + " - "
				+ this.prevBillRefResets.toString());
	...
	public void expectedInvoiceGenerated(String expectedFileName, String configFileName) throws Throwable {
		...
		Map<String, String> tableKeyFieldMapping = getTableKeyFieldMappingFromConfigFile(configFileName);
		...
		/*
		 * Reporter.addStepLog("New invoice generated: " + cmfAfterTest.getValue(0,
		 * "prev_bill_refno").toString());
		 */

	}

	@Then("^expected files should be generated based on (.*) file$")
	public void expectedFilesGenerated(String configFileName) throws Throwable {
		...
		getHeaderAndDataDynamically(headerList, iTables, tableFieldMapping, tableKeyFieldMapping, "expected");
	...
	@Then("^actual files should be generated based on (.*) file$")
	public void actualFilesGenerated(String configFileName) throws Throwable {
		...
		getHeaderAndDataDynamically(headerList, iTables, tableFieldMapping, tableKeyFieldMapping, "actual");
	...
	@Then("^compare the (.*) and (.*) files dynamically and verify the results$")
	public void compareFilesDynamically(String expectedFile, String actualFile) {
		compareDynamically("src/test/resources/" + expectedFile, "src/test/resources/" + actualFile);
	}
	 ...
	 * @throws IOException
	 */
	private Map<String, List<String>> getTableFieldMappingFromConfigFile(String configFileName)
		...
		while ((read_line = reader.readLine()) != null) {
			if (!read_line.startsWith("KEYFIELDS")) {
	...
	/**
	 * Get each table's key fields to join on, using given config file
	 * 
	 * @return
	 * @throws FileNotFoundException
	 * @throws IOException
	 */
	private Map<String, String> getTableKeyFieldMappingFromConfigFile(String configFileName)
			throws FileNotFoundException, IOException {
		File config_file = new File("src/test/resources/" + configFileName);
		BufferedReader reader = new BufferedReader(new FileReader(config_file));

		String read_line = null;
		Map<String, String> tableKeyFieldMapping = new HashMap<String, String>();
		while ((read_line = reader.readLine()) != null) {

			if (read_line.startsWith("KEYFIELDS")) {
				String[] tableKeyFields = read_line.split("\\|");

				String[] fields = tableKeyFields[1].split(":");
				tableKeyFieldMapping.put(fields[0], fields[1]);
			}
		}

		return tableKeyFieldMapping;
	}
	 ...
	 * @throws Exception
	 */
	private void getHeaderAndData(List<String> headerList, List<ITable> iTables,
			Map<String, List<String>> tableFieldMapping) throws Exception {
		String table = null;
		...
		for (String key : tableFieldMapping.keySet()) {
			...
			if (table.equals("CMF")) {
				cmfAfterTest = databaseTester.getConnection().createQueryTable(table,
						"select " + headers + " from " + table + " where account_no = " + this.accountNo);

				iTables.add(cmfAfterTest);

				assertEquals(1, cmfAfterTest.getRowCount());
				Assert.assertNotEquals(this.prevBillRefNo, cmfAfterTest.getValue(0, "prev_bill_refno").toString());
			}

			ITable billInvoice = null;
			if (table.equals("BILL_INVOICE")) {
				billInvoice = databaseTester.getConnection().createQueryTable(table,
						"select " + headers + " from " + table + " where bill_ref_no = " + this.prevBillRefNo
								+ " and bill_ref_resets = " + this.prevBillRefResets);
	...
	/**
	 * Get the header and data to be added to a file dynamically.
	 * 
	 * @param headerList
	 * @param iTables
	 * @param tableFieldMapping
	 * @throws Exception
	 */
	private void getHeaderAndDataDynamically(List<String> headerList, List<ITable> iTables,
			Map<String, List<String>> tableFieldMapping, Map<String, String> tableKeyFieldMapping, String mode)
			throws Exception {
		String table = null;
		ITable iTable = null;
		...
		for (String key : tableFieldMapping.keySet()) {
			...
			String whereClause = " where " + tableKeyFieldMapping.get(table);

			iTable = databaseTester.getConnection().createQueryTable(table,
					"select " + headers + " from " + table + whereClause);

			iTables.add(iTable);
			headerList.add(headers);

			createFileDynamically(headers, iTable, table, mode);

		}
	}

	/**
	 * Creates the csv file with given headers and data with the specified file
	 * name.
	 * 
	 * @param headers
	 * @param iTables
		...
		try {
			...
			for (int i = 0; i < iTables.size(); i++) {
				if (iTables.get(i).getTableMetaData().getTableName().equals("CMF")) {
					line.append("," + String.valueOf(iTables.get(i).getValue(0, "account_no")) + ","
							+ String.valueOf(iTables.get(i).getValue(0, "prev_bill_refno")) + ","
							+ String.valueOf(iTables.get(i).getValue(0, "prev_bill_ref_resets")));
	...
	/**
	 * Creates (multiple) csv files with given headers and data with the specified
	 * file name.
	 * 
	 * @param headers
	 * @param iTables
	 ...
	 * @throws Exception
	 */
	private void createFileDynamically(String headers, ITable iTable, String tableName, String mode) throws Exception {
		File fc = null;
		BufferedWriter writer = null;

		try {

			String filePath = "src/test/resources/" + tableName + "_" + mode + ".csv";
			...
			writer = new BufferedWriter(new FileWriter(fc, true));
			writer.write(headers.trim());
			writer.newLine();

			for (int row = 0; row < iTable.getRowCount(); row++) {
				StringBuffer line = new StringBuffer();
				for (int col = 0; col < iTable.getTableMetaData().getColumns().length; col++) {
					line.append(String.valueOf(
							iTable.getValue(row, iTable.getTableMetaData().getColumns()[col].getColumnName())));
					line.append(", ");
			...
			for (int j = 1; j < actualFieldList.size(); j++) { // verify values for each row in actual list
				for (int column = 1; column < numOfcolumns + 1; column++) {
					if (expectedFieldList.get(0)[column - 1].equals("account_no")) {
						...
						Assert.assertTrue(Integer.valueOf(expectedFieldList.get(i)[column]) < Integer
								.valueOf(actualFieldList.get(j)[column]));
	...
	/**
	 * Compare the given expected and actual files in a dynamic manner.
	 * 
	 * @param expectedFilePath
	 * @param actualFilePath
	 */
	private void compareDynamically(String expectedFilePath, String actualFilePath) {
		List<String[]> expectedFieldList = getDataFromFile(expectedFilePath);
		List<String[]> actualFieldList = getDataFromFile(actualFilePath);
		...
		for (int i = 1; i < expectedFieldList.size(); i++) { // verify values for each row in expected list
			for (int column = 0; column < numOfcolumns; column++) {
				Assert.assertTrue(expectedFieldList.get(i)[column].equals(actualFieldList.get(i)[column]));
			}
		}
	}
